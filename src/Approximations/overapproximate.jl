...
"""
function overapproximate(P::AbstractSparsePolynomialZonotope{N}, ::Type{<:VPolytope}) where {N}
    n = dim(P)
    c = center(P)
    G = genmat_dep(P)
    GI = genmat_indep(P)
    ...
    if !isempty(H)
        SPZ₂ = SimpleSparsePolynomialZonotope(c, G[:, H], E[:, H])
...
# Extended help
    ...
    overapproximate(CH::ConvexHull{N,<:AbstractSparsePolynomialZonotope,
                                     <:AbstractSparsePolynomialZonotope},
                    ::Type{<:SparsePolynomialZonotope}) where {N}

### Algorithm

This method implements [Kochdumper21a; 3.1.28](@citet).
"""
...
function overapproximate(lm::LinearMap{N,S,NM,MAT},
    ...
    SP = SparsePolynomialZonotope(center(P), genmat_dep(P), zeros(N, n, 0), expmat(P),
                                  indexvector(P))
    Pd = linear_map(MZ, SP)
    ...
    return SparsePolynomialZonotope(center(Pd), genmat_dep(Pd), genmat(Zi), expmat(Pd),
                                    indexvector(Pd))
end
...
"""
function overapproximate(lm::LinearMap{N,S,NM,MAT},
                         T::Type{<:SparsePolynomialZonotope}) where {N,
                                                                     S<:SparsePolynomialZonotope{N},
                                                                     NM,
                                                                     MAT<:MatrixZonotopeProduct{NM}}
